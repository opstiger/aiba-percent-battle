...
const root=process.cwd(),out=path.join(root,process.env.AIBA_QA_OUT||'artifacts/shoe-bred-20260916');fs.mkdirSync(out,{recursive:true});
...
try{
 ...
 await page.addScriptTag({path:path.join(root,'tools/shoe-fit-lab.js')});await page.addScriptTag({path:path.join(root,'tools/shoe-bred-preview.js')});
 await page.evaluate(()=>{
  ...
  window.last=AIBAShoeFitLab.build({highTop:true});AIBABredPreview.style(last);labScene.add(last);
 ...
 report.colorStudy=await page.evaluate(()=>{let triangles=0;last.traverse(m=>{if(m.isMesh)triangles+=(m.geometry.index?m.geometry.index.count:m.geometry.attributes.position.count)/3;});return {triangles,palette:AIBABredPreview.palette};});
 report.fits=[];
 for(const length of [.42,.44,.46]){
  const data=await page.evaluate(length=>{
   ...
   const before=rigSnapshot();const f=AIBAShoeFitLab.apply(subject,{length});subject.footRoots.forEach(f=>AIBABredPreview.style(f.getObjectByName('ShoeLastDebug')));const after=rigSnapshot();
