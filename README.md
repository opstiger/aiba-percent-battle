...
Developer notes — muting audio in tests, the appearance viewer, motion-control debugging, replaying a seeded game, per-mode smoke checks — live in [`docs/DEVELOPMENT.zh-CN.md`](docs/DEVELOPMENT.zh-CN.md) (written in Chinese).

## Project layout

- `viewer.html` — inspect a single player or gear item on a neutral backdrop, with no game logic loaded (see [developer notes](docs/DEVELOPMENT.zh-CN.md)).
- `tools/capture.mjs` — fixed-camera screenshot rig; output lands in `captures/` (not tracked).
- `index.html` — the playable entry: a ~200-line modular shell that loads the game from `src/` (cutover completed in v2.0).
- `block-3pt-kingv2.28.0-modular.html` — current versioned snapshot, kept identical to `index.html`. Older snapshots live in git history and tags, not in the tree.
