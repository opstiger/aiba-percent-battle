...
/* 0(关闭)。与 core.js 保持一致:低角度掠射 rim 是地板洗白的唯一来源,
   且对人物无贡献。详见 core.js 的撤销说明。 */
...
const ARENA_KEY_I=0.35,ARENA_FILL_I=0.32;
...
const arenaLights=ARENA_LIGHT_LAYOUT.map(p=>{
  /* angle .70→.80、penumbra .70→.92,都是为了消掉"局部爆白"。
     原来 angle .70 在 11.5m 高处只铺开约 9.7m 半径,而灯间距是左右 17m / 前后 12m
     —— 四块光斑各自独立、互不重叠,地面于是出现 4 个明显亮区,
     再叠上地板的 specular 就在每个亮区中心烧出白点。
     真实馆是几十盏灯把光斑糊成一片均匀照明,所以这里**让光斑交叠**:
     angle 放宽到 .80(半径约 11.8m,相邻开始重叠),penumbra 拉到 .92
     让中心到边缘的过渡非常平缓,热点自然消失。
     注意 penumbra 变大会摊薄总能量,所以强度另由 applyArenaLightMix 分配。 */
  const l=new THREE.SpotLight(0xfff6e8,0.20,46,0.80,0.92,0.9);
